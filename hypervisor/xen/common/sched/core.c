...
ret_t do_sched_op(int cmd, XEN_GUEST_HANDLE_PARAM(void) arg)
{
    ...
    case SCHEDOP_deferred_preempt_ready:
    {
        struct vcpu *v = current;

        /* The guest must leave the protected interval before notifying us. */
        if ( ACCESS_ONCE(vcpu_info(v, preempt_count)) )
        {
            ret = -EBUSY;
            break;
        }
        ...
        /*
         * Do not call vcpu_yield(): that changes scheduler policy and can
         * place an RT-sensitive VCPU behind unrelated runnable work. Merely
         * revisit the scheduling decision that Xen previously deferred.
         * yield_count is cleared after that scheduling decision completes.
         */
        smp_mb();
        if ( ACCESS_ONCE(vcpu_info(v, yield_count)) )
            raise_softirq(SCHEDULE_SOFTIRQ);
        break;
    }
...
/*
 * Guest unlock normally wakes the scheduler through
 * SCHEDOP_deferred_preempt_ready. A fixed one-shot deadline is only a
 * watchdog for a lost notification or an abnormally long critical section.
 */
#define SCHED_GUEST_PREEMPT_WATCHDOG_MS 1

/*
 * Fast path for the common single-VCPU scheduling unit.  This runs before
 * taking the scheduler lock so that a scheduling interrupt which hits a
 * guest lock holder does not enter the policy scheduler merely to discover
 * that the decision has to be deferred.
 *
 * Complex scheduling units are deliberately left to the locked path below.
 */
static bool sched_defer_current_preemption_early(
    struct vcpu *v, s_time_t *retry_deadline)
{
    uint8_t pending;
    s_time_t deadline;
    s_time_t now;
    ...
    *retry_deadline = 0;
    ...
    if ( is_idle_vcpu(v) || !v->is_running ||
         !ACCESS_ONCE(vcpu_info(v, preempt_count)) )
        return false;
    if ( !vcpu_runnable(v) || ACCESS_ONCE(v->force_context_switch) )
        return false;

    if ( unit_scheduler(v->sched_unit)->sched_id == XEN_SCHEDULER_NULL )
        return false;

    now = NOW();
    pending = ACCESS_ONCE(vcpu_info(v, yield_count));
    if ( !pending )
    {
        ACCESS_ONCE(vcpu_info(v, yield_count)) = 1;
        v->guest_preempt_deadline =
            now + MILLISECS(SCHED_GUEST_PREEMPT_WATCHDOG_MS);
    }
    deadline = v->guest_preempt_deadline;

    /* Pair with the guest's full barrier on the outermost unlock. */
    smp_mb();
    if ( !ACCESS_ONCE(vcpu_info(v, preempt_count)) )
        return false;

    /* The locked path handles an expired watchdog and its warning. */
    if ( deadline <= now )
        return false;
...
/*
 * Return true when a running VCPU in @unit is in a guest critical section.
 * Guest unlock requests an immediate retry; a fixed deadline is retained as
 * a watchdog. Xen clears yield_count after the scheduling decision.
 */
static bool sched_defer_guest_preemption(struct sched_unit *unit,
                                          s_time_t *retry_deadline)
{
    struct vcpu *v;
    bool defer = false;
    s_time_t now = NOW();
    ...
    *retry_deadline = 0;
    ...
    /*
     * The null scheduler never time-shares its statically assigned units.
     * In particular, delaying a mandatory context switch here can prevent a
     * migration from reaching unit_context_saved().  Check the unit's
     * scheduler rather than the current pCPU's scheduler, as the latter can
     * temporarily be the idle scheduler while cpupools are being changed.
     */
    if ( unit_scheduler(unit)->sched_id == XEN_SCHEDULER_NULL )
        return false;
    ...
    for_each_sched_unit_vcpu ( unit, v )
    {
        ...
        if ( !v->is_running || !ACCESS_ONCE(vcpu_info(v, preempt_count)) )
            continue;
        ...
        /*
         * Close the race with the guest's outermost unlock. If the guest
         * cleared preempt_count before or while we published yield_count,
         * do not defer: this scheduling pass can proceed directly.
         * Otherwise the guest observes yield_count after its matching full
         * barrier and issues SCHEDOP_deferred_preempt_ready.
         */
        smp_mb();
        if ( !ACCESS_ONCE(vcpu_info(v, preempt_count)) )
            continue;
        ...
        if ( deadline <= now )
        {
            printk(XENLOG_WARNING
                   "GUEST PREEMPT: forcing schedule of d%uv%u on CPU%u "
                   "after %u ms (preempt_count=%u)\n",
                   v->domain->domain_id, v->vcpu_id, smp_processor_id(),
                   SCHED_GUEST_PREEMPT_WATCHDOG_MS,
                   ACCESS_ONCE(vcpu_info(v, preempt_count)));
            return false;
        }

        if ( !*retry_deadline || deadline < *retry_deadline )
            *retry_deadline = deadline;
        defer = true;
    }
    return defer;
                                 ...
                                 bool reset_idle_unit, s_time_t now)
{
    if ( !is_idle_vcpu(vprev) &&
         ACCESS_ONCE(vcpu_info(vprev, yield_count)) )
    {
        ACCESS_ONCE(vcpu_info(vprev, yield_count)) = 0;
        vprev->guest_preempt_deadline = 0;
    }
                                                   ...
                                                   s_time_t now)
{
    ...
    while ( prev->rendezvous_in_cnt )
    {
        if ( v && v->force_context_switch )
        {
            ...
            if ( sched_defer_guest_preemption(prev, &defer_deadline) )
            {
                prev->rendezvous_in_cnt++;
                atomic_set(&prev->rendezvous_out_cnt, 0);
                set_timer(&sr->s_timer, defer_deadline);
...
static void cf_check sched_slave(void)
{
    ...
    if ( v && v->force_context_switch )
    {
        if ( sched_defer_guest_preemption(prev, &defer_deadline) )
        {
            set_timer(&get_sched_res(cpu)->s_timer, defer_deadline);
            pcpu_schedule_unlock_irq(lock, cpu);
            rcu_read_unlock(&sched_res_rculock);
...
static void cf_check schedule(void)
{
    ...
    /*
     * For a single-VCPU scheduling unit there is no rendezvous peer whose
     * state has to be inspected under the scheduler lock.  Defer the same
     * decision here and avoid the scheduler lock and policy scheduler.  Keep
     * the locked check below for races and all complex paths.
     */
    if ( gran == 1 && !ACCESS_ONCE(prev->rendezvous_in_cnt) &&
         sched_defer_current_preemption_early(vprev, &defer_deadline) )
    {
        set_timer(&sr->s_timer, defer_deadline);
        rcu_read_unlock(&sched_res_rculock);
        return;
    ...
    /* The resource may have changed before the scheduler lock was taken. */
    sr = get_sched_res(cpu);
    gran = sr->granularity;
