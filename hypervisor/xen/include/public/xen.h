...
struct vcpu_info {
    ...
    /*
     * The guest increments preempt_count while holding a spin lock.  If Xen
     * tries to preempt the VCPU in that interval, it leaves the VCPU running
     * and sets yield_count. On the outermost unlock the guest issues
     * SCHEDOP_deferred_preempt_ready, which revisits the pending scheduling
     * decision without voluntarily yielding the VCPU. Xen clears yield_count
     * after completing a scheduling decision.
     *
     * These fields consume padding which existed before evtchn_pending_sel;
     * keep them here so that the remainder of vcpu_info retains its ABI.
     */
