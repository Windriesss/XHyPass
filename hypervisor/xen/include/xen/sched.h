...
#ifdef CONFIG_ARM
/*
 * Interrupt-delivery ownership for an XHyPass vCPU.  Remote physical-IRQ
 * handlers observe the transitional states so that an IRQ acknowledged on
 * one pCPU cannot be injected into a vGIC being disabled on another pCPU.
 */
enum xhypass_irq_mode {
    XHYPASS_IRQ_DYN = 0,
    XHYPASS_IRQ_TO_RTO,
    XHYPASS_IRQ_RTO,
    XHYPASS_IRQ_TO_DYN,
...
#endif
#ifdef CONFIG_ARM
    enum xhypass_irq_mode irq_mode;
    spinlock_t irq_mode_lock;
    unsigned int rto_event_sgi;
#endif
    s_time_t guest_preempt_deadline;
};
