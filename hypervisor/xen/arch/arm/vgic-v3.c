...
static bool vgic_v3_emulate_rto_pmr(struct cpu_user_regs *regs, uint64_t *r,
                                    bool read)
{
    if ( ACCESS_ONCE(current->irq_mode) != XHYPASS_IRQ_RTO )
        return false;
    if ( read )
        *r = READ_SYSREG(ICC_PMR_EL1);
    else
        WRITE_SYSREG(*r, ICC_PMR_EL1);
    return true;
}

static bool vgic_v3_emulate_rto_dir(struct cpu_user_regs *regs, uint64_t *r,
                                    bool read)
{
    if ( ACCESS_ONCE(current->irq_mode) != XHYPASS_IRQ_RTO || read )
        return false;
    WRITE_SYSREG(*r, ICC_DIR_EL1);
    isb();
    return true;
}

static bool vgic_v3_emulate_rto_rpr(struct cpu_user_regs *regs, uint64_t *r,
                                    bool read)
{
    if ( ACCESS_ONCE(current->irq_mode) != XHYPASS_IRQ_RTO || !read )
        return false;
    *r = READ_SYSREG(ICC_RPR_EL1);
    return true;
}

static bool vgic_v3_emulate_rto_ctlr(struct cpu_user_regs *regs, uint64_t *r,
                                     bool read)
{
    if ( ACCESS_ONCE(current->irq_mode) != XHYPASS_IRQ_RTO )
        return false;
    if ( read )
        *r = READ_SYSREG(ICC_CTLR_EL1);
    else
    {
        WRITE_SYSREG(*r, ICC_CTLR_EL1);
        isb();
    }
    return true;
}
...
static bool vgic_v3_emulate_sysreg(struct cpu_user_regs *regs, union hsr hsr)
{
    ...
    {
    case HSR_SYSREG_ICC_PMR_EL1:
        return vreg_emulate_sysreg(regs, hsr, vgic_v3_emulate_rto_pmr);

    case HSR_SYSREG_ICC_DIR_EL1:
        return vreg_emulate_sysreg(regs, hsr, vgic_v3_emulate_rto_dir);

    case HSR_SYSREG_ICC_RPR_EL1:
        return vreg_emulate_sysreg(regs, hsr, vgic_v3_emulate_rto_rpr);

    case HSR_SYSREG_ICC_CTLR_EL1:
        return vreg_emulate_sysreg(regs, hsr, vgic_v3_emulate_rto_ctlr);
