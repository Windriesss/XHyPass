...
#define ICH_VMCR_VBPR1_MASK  0x7
#define ICC_IGRPEN1_EN		0x1

static bool gicv3_rto_irq(struct vcpu *v, unsigned int virq,
                          struct pending_irq **pending)
{
    struct pending_irq *p;

    if ( virq < NR_GIC_LOCAL_IRQS || virq >= vgic_num_irqs(v->domain) ||
         !test_bit(virq, v->domain->arch.vgic.allocated_irqs) ||
         vgic_get_target_vcpu(v->domain->vcpu[0], virq) != v )
        return false;

    p = spi_to_pending(v->domain, virq);
    if ( !p->desc || p->desc->irq >= NR_IRQS )
        return false;
...
/* Caller holds v->arch.vgic.lock. */
static bool gicv3_rto_vgic_owns_irq(const struct pending_irq *p)
{
...
/* Serialize with a remote do_IRQ() which may already have read IAR. */
static void gicv3_rto_mask_irqs(struct vcpu *v, unsigned long *enabled)
{
    unsigned int virq;
    struct pending_irq *p;

    bitmap_zero(enabled, NR_IRQS);
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        if ( !gicv3_rto_irq(v, virq, &p) )
            continue;

        spin_lock(&p->desc->lock);
        if ( gicv3_peek_irq(p->desc, GICD_ISENABLER) )
            set_bit(virq, enabled);
        gicv3_mask_irq(p->desc);
        spin_unlock(&p->desc->lock);
    }
}

static void gicv3_rto_unmask_irqs(struct vcpu *v,
                                  const unsigned long *enabled)
{
    unsigned int virq;
    struct pending_irq *p;
    ...
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        if ( !test_bit(virq, enabled) ||
             !gicv3_rto_irq(v, virq, &p) )
            continue;

        spin_lock(&p->desc->lock);
        gicv3_unmask_irq(p->desc);
        spin_unlock(&p->desc->lock);
    }
}

/* Local interrupts cannot be migrated; retry until no handler is active. */
static int gicv3_rto_local_active(void)
{
    unsigned int irq;
    ...
    for ( irq = 0; irq < NR_GIC_LOCAL_IRQS; irq++ )
    {
        ...
        spin_lock(&desc->lock);
        active = gicv3_peek_irq(desc, GICD_ISACTIVER);
        pending = gicv3_peek_irq(desc, GICD_ISPENDR);
        spin_unlock(&desc->lock);

        if ( active )
            return pending ? -EAGAIN : -EBUSY;
    }
    return 0;
}

/*
 * During DYN-to-RTO, a hardware PPI acknowledged by Xen may be Physical
 * Active while the corresponding guest occurrence is still Virtual Pending.
 * That Xen-owned occurrence can be deactivated and re-pended by the drain
 * phase.  Active SGIs and the software event-channel PPI are not attributed
 * this way and therefore remain retry conditions.
 */
static int gicv3_rto_preflight_local(struct vcpu *v)
{
    unsigned int virq;

    for ( virq = 0; virq < NR_GIC_LOCAL_IRQS; virq++ )
    {
        struct irq_desc *desc = irq_to_desc(virq);
        struct pending_irq *p = irq_to_pending(v, virq);
        bool represented, active, pending, transferable_active;

        spin_lock(&desc->lock);
        spin_lock(&v->arch.vgic.lock);
        represented = gicv3_rto_vgic_owns_irq(p);
        active = gicv3_peek_irq(desc, GICD_ISACTIVER);
        pending = gicv3_peek_irq(desc, GICD_ISPENDR);
        transferable_active = represented && virq >= NR_GIC_SGI &&
                              virq != v->domain->arch.evtchn_irq;

        if ( test_bit(GIC_IRQ_GUEST_ACTIVE, &p->status) ||
             (active && pending) || (active && !transferable_active) )
        {
            spin_unlock(&v->arch.vgic.lock);
            spin_unlock(&desc->lock);
            return active && pending ? -EAGAIN : -EBUSY;
        }
...
/*
 * The distributor is not mapped to the guest, but an enabled SPI routed to
 * this pCPU can still arrive through its physical CPU interface.  Require
 * management code to move every non-target SPI away before entering RTO.
 */
static int gicv3_rto_check_non_target_routes(struct vcpu *v)
{
    DECLARE_BITMAP(target, NR_IRQS);
    const uint64_t affinity = gicv3_mpidr_to_affinity(v->processor);
    struct pending_irq *p;
    unsigned int irq, virq;

    bitmap_zero(target, NR_IRQS);
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
        if ( gicv3_rto_irq(v, virq, &p) )
            set_bit(p->desc->irq, target);

    for ( irq = NR_GIC_LOCAL_IRQS;
          irq < min(gicv3_info.nr_lines, (unsigned int)NR_IRQS); irq++ )
    {
        struct irq_desc *desc;
        uint64_t route;
        bool enabled;

        if ( test_bit(irq, target) )
            continue;

        desc = irq_to_desc(irq);
        spin_lock(&desc->lock);
        enabled = gicv3_peek_irq(desc, GICD_ISENABLER);
        route = readq_relaxed_non_atomic(GICD + GICD_IROUTER + irq * 8);
        spin_unlock(&desc->lock);

        if ( !enabled )
            continue;
        if ( (route & GICD_IROUTER_SPI_MODE_ANY) ||
             ((route & ~GICD_IROUTER_SPI_MODE_ANY) == affinity) )
            return -EBUSY;
    }
    return 0;
}
...
/*
 * Validate the handoff without modifying either state machine.  In
 * particular, Physical Active without a vGIC representation means that a
 * remote pCPU has read IAR but has not completed do_IRQ().  The caller must
 * roll back and retry; synthesizing Pending here could duplicate delivery.
 */
static int gicv3_rto_preflight(struct vcpu *v)
{
    struct pending_irq *p;
    struct gic_lr lr;
    unsigned int n, virq;
    ...
    for ( n = 0; n < gicv3_info.nr_lrs; n++ )
    {
        ...
        if ( lr.pending )
        {
            p = irq_to_pending(v, lr.virq);
            if ( lr.virq < NR_GIC_LOCAL_IRQS )
            {
                /*
                 * Local virtual and physical INTIDs are identical in the
                 * XHyPass execution model.  A Pending-only SGI/PPI can be
                 * materialized on the paired pCPU after this preflight.
                 */
                if ( !p || list_empty(&p->inflight) ||
                     !test_bit(GIC_IRQ_GUEST_VISIBLE, &p->status) ||
                     p->lr != n ||
                     (lr.hw_status && lr.hw.pirq != lr.virq) )
                {
                    rc = -EOPNOTSUPP;
                    goto out_vgic;
                }
            ...
            if ( !lr.hw_status || !p || !p->desc ||
                 p->desc->irq != lr.hw.pirq || lr.hw.pirq >= NR_IRQS ||
                 !test_bit(GIC_IRQ_GUEST_VISIBLE, &p->status) ||
                 p->lr != n ||
                 vgic_get_target_vcpu(v->domain->vcpu[0], lr.virq) != v )
            {
                rc = -EOPNOTSUPP;
                goto out_vgic;
            }
    ...
    list_for_each_entry ( p, &v->arch.vgic.inflight_irqs, inflight )
    {
        ...
        if ( p->irq < NR_GIC_LOCAL_IRQS )
            continue;
        if ( !p->desc || p->desc->irq >= NR_IRQS ||
             vgic_get_target_vcpu(v->domain->vcpu[0], p->irq) != v )
        {
            rc = -EOPNOTSUPP;
            goto out_vgic;
        }
    ...
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        bool represented, active, pending;
        ...
        spin_lock(&v->arch.vgic.lock);
        represented = gicv3_rto_vgic_owns_irq(p);
        active = gicv3_peek_irq(p->desc, GICD_ISACTIVER);
        pending = gicv3_peek_irq(p->desc, GICD_ISPENDR);
        spin_unlock(&v->arch.vgic.lock);
        spin_unlock(&p->desc->lock);
...
/*
 * Convert every Pending-only local vGIC occurrence into an equivalent
 * physical notification on the paired pCPU.  The caller has disabled local
 * IRQ handling and holds irq_mode_lock, so no guest SGI/event delivery can
 * race this ownership handoff.
 */
static void gicv3_rto_drain_local_vgic(struct vcpu *v)
{
    unsigned int virq;
    ...
    for ( virq = 0; virq < NR_GIC_LOCAL_IRQS; virq++ )
    {
        struct irq_desc *desc = irq_to_desc(virq);
        struct pending_irq *p = irq_to_pending(v, virq);
        bool represented, visible, active;
        ...
        visible = test_bit(GIC_IRQ_GUEST_VISIBLE, &p->status);
        active = gicv3_peek_irq(desc, GICD_ISACTIVER);

        if ( virq == v->domain->arch.evtchn_irq )
        {
            /*
             * Event data remains in the shared event-channel structures.
             * Materialize the outstanding notification as the RTO event SGI
             * before removing its virtual PPI representation.
             */
            if ( ACCESS_ONCE(vcpu_info(v, evtchn_upcall_pending)) )
                send_SGI_one(v->processor,
                             (enum gic_sgi)v->rto_event_sgi);
        }
        else if ( virq < NR_GIC_SGI )
        {
            send_SGI_one(v->processor, (enum gic_sgi)virq);
        }
        else
        {
            /* PPIs are banked in the current pCPU's Redistributor. */
            if ( active )
                gicv3_set_active_state(desc, false);
            gicv3_set_pending_state(desc, true);
        }

        /* The physical notification must be visible before its vGIC copy. */
        dsb(sy);

        if ( visible && p->lr != GIC_INVALID_LR )
        {
            gicv3_clear_lr(p->lr);
            clear_bit(p->lr, &this_cpu(lr_mask));
        }
...
/* Convert every Xen-owned occurrence to one physical Pending occurrence. */
static int gicv3_rto_drain_vgic(struct vcpu *v)
{
    unsigned int virq;
    struct pending_irq *p;
    bool represented, active, pending;
    ...
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        ...
        if ( unlikely(active && !represented) )
        {
            spin_unlock(&v->arch.vgic.lock);
            spin_unlock(&p->desc->lock);
            v->arch.lr_mask = this_cpu(lr_mask);
        ...
        if ( active )
            gicv3_set_active_state(p->desc, false);
        gicv3_set_pending_state(p->desc, true);
...
static void gicv3_rto_route_irqs(struct vcpu *v)
{
    unsigned int virq;
    struct pending_irq *p;
    ...
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        if ( !gicv3_rto_irq(v, virq, &p) )
            continue;
        irq_set_affinity(p->desc, cpumask_of(v->processor));
        gicv3_set_irq_priority(p->desc, p->priority);
    }
    dsb(sy);
    gicv3_dist_wait_for_rwp();
}

static void gicv3_rto_save_cpuif(struct vcpu *v)
{
    v->arch.rto_gic.hcr_el2 = READ_SYSREG(HCR_EL2);
    v->arch.rto_gic.ich_hcr_el2 = READ_SYSREG(ICH_HCR_EL2);
    v->arch.rto_gic.icc_ctlr_el1 = READ_SYSREG(ICC_CTLR_EL1);
    v->arch.rto_gic.icc_igrpen1_el1 = READ_SYSREG(ICC_IGRPEN1_EL1);
    v->arch.rto_gic.icc_pmr_el1 = READ_SYSREG(ICC_PMR_EL1);
    v->arch.rto_gic.icc_bpr1_el1 = READ_SYSREG(ICC_BPR1_EL1);
}

static void gicv3_rto_vmcr_to_phys(void)
{
    register_t vmcr = READ_SYSREG(ICH_VMCR_EL2);
    register_t ctlr = READ_SYSREG(ICC_CTLR_EL1);

    ctlr &= ~GICC_CTLR_EL1_EOImode_drop;
    if ( vmcr & ICH_VMCR_EOI )
        ctlr |= GICC_CTLR_EL1_EOImode_drop;
    WRITE_SYSREG(ctlr, ICC_CTLR_EL1);
    WRITE_SYSREG((vmcr >> ICH_VMCR_PRIORITY_SHIFT) &
                 ICH_VMCR_PRIORITY_MASK, ICC_PMR_EL1);
    WRITE_SYSREG((vmcr >> ICH_VMCR_VBPR1_SHIFT) &
                 ICH_VMCR_VBPR1_MASK, ICC_BPR1_EL1);
    WRITE_SYSREG((vmcr & ICH_VMCR_VENG1) ? ICC_IGRPEN1_EN : 0,
                 ICC_IGRPEN1_EL1);
    isb();
}

static void gicv3_rto_phys_to_vmcr(struct vcpu *v)
{
    register_t vmcr = READ_SYSREG(ICH_VMCR_EL2);
    register_t ctlr = READ_SYSREG(ICC_CTLR_EL1);

    vmcr &= ~(ICH_VMCR_EOI | ICH_VMCR_VENG1 |
              ((register_t)ICH_VMCR_PRIORITY_MASK <<
               ICH_VMCR_PRIORITY_SHIFT) |
              ((register_t)ICH_VMCR_VBPR1_MASK << ICH_VMCR_VBPR1_SHIFT));
    if ( ctlr & GICC_CTLR_EL1_EOImode_drop )
        vmcr |= ICH_VMCR_EOI;
    if ( READ_SYSREG(ICC_IGRPEN1_EL1) & ICC_IGRPEN1_EN )
        vmcr |= ICH_VMCR_VENG1;
    vmcr |= (READ_SYSREG(ICC_PMR_EL1) & ICH_VMCR_PRIORITY_MASK) <<
            ICH_VMCR_PRIORITY_SHIFT;
    vmcr |= (READ_SYSREG(ICC_BPR1_EL1) & ICH_VMCR_VBPR1_MASK) <<
            ICH_VMCR_VBPR1_SHIFT;
    WRITE_SYSREG(vmcr, ICH_VMCR_EL2);
    v->arch.gic.v3.vmcr = vmcr;
}

static int gicv3_rto_log_failure(const struct vcpu *v, const char *direction,
                                 const char *stage, int rc)
{
    gprintk(XENLOG_WARNING,
            "XHyPass %s rejected: vCPU%u pCPU%u stage=%s rc=%d mode=%u\n",
            direction, v->vcpu_id, v->processor, stage, rc,
            ACCESS_ONCE(v->irq_mode));

    return rc;
}

static int gicv3_rto_check_event_sgi(unsigned int event_sgi)
{
    struct irq_desc *desc = irq_to_desc(event_sgi);
    int rc = 0;

    /* Dynamic SGI users are registered during platform initialization. */
    spin_lock(&desc->lock);
    if ( desc->action || test_bit(_IRQ_GUEST, &desc->status) )
        rc = -EBUSY;
    spin_unlock(&desc->lock);
...
static int gicv3_enter_rto(unsigned int event_sgi)
{
    ...
    if ( local_irq_is_enabled() )
        return gicv3_rto_log_failure(v, "DYN->RTO",
                                     "el2-local-irqs-enabled", -EPERM);
    if ( v->processor != smp_processor_id() )
        return gicv3_rto_log_failure(v, "DYN->RTO", "wrong-pcpu", -EPERM);
    ...
    rc = gicv3_rto_check_event_sgi(event_sgi);
    if ( rc )
        return gicv3_rto_log_failure(v, "DYN->RTO", stage, rc);

    spin_lock(&v->irq_mode_lock);
    stage = "claim-mode";
    if ( cmpxchg(&v->irq_mode, XHYPASS_IRQ_DYN,
                 XHYPASS_IRQ_TO_RTO) != XHYPASS_IRQ_DYN )
    {
        rc = -EBUSY;
        goto out_unlock;
    }
    v->rto_event_sgi = event_sgi;
    smp_mb();
    gicv3_rto_mask_irqs(v, enabled);

    stage = "local-active-check";
    rc = gicv3_rto_preflight_local(v);
    if ( rc )
        goto abort;
    stage = "non-target-route-check";
    rc = gicv3_rto_check_non_target_routes(v);
    if ( rc )
        goto abort;
    stage = "vgic-preflight";
    rc = gicv3_rto_preflight(v);
    if ( rc )
        goto abort;
    gicv3_rto_route_irqs(v);
    stage = "vgic-drain";
    rc = gicv3_rto_drain_vgic(v);
    if ( rc )
        goto abort;
    gicv3_rto_drain_local_vgic(v);
    gicv3_rto_save_cpuif(v);
    gicv3_rto_vmcr_to_phys();

    WRITE_SYSREG(GICH_HCR_TC, ICH_HCR_EL2);
    hcr = (v->arch.rto_gic.hcr_el2 | HCR_FMO) &
          ~(HCR_IMO | HCR_TWI | HCR_TWE);
    WRITE_SYSREG(hcr, HCR_EL2);
    v->arch.hcr_el2 = hcr;
    isb();

    smp_wmb();
    ACCESS_ONCE(v->irq_mode) = XHYPASS_IRQ_RTO;
...
out_unlock:
    spin_unlock(&v->irq_mode_lock);
    if ( rc )
        return gicv3_rto_log_failure(v, "DYN->RTO", stage, rc);
    return rc;
}

static int gicv3_rto_phys_active(struct vcpu *v)
{
    unsigned int virq;
    struct pending_irq *p;
    ...
    for ( virq = NR_GIC_LOCAL_IRQS; virq < vgic_num_irqs(v->domain); virq++ )
    {
        if ( !gicv3_rto_irq(v, virq, &p) )
            continue;
        spin_lock(&p->desc->lock);
        if ( gicv3_peek_irq(p->desc, GICD_ISACTIVER) )
        {
            bool pending = gicv3_peek_irq(p->desc, GICD_ISPENDR);
...
static int gicv3_exit_rto(void)
{
    ...
    if ( local_irq_is_enabled() )
        return gicv3_rto_log_failure(v, "RTO->DYN",
                                     "el2-local-irqs-enabled", -EPERM);
    if ( v->processor != smp_processor_id() )
        return gicv3_rto_log_failure(v, "RTO->DYN", "wrong-pcpu", -EPERM);

    spin_lock(&v->irq_mode_lock);
    stage = "claim-mode";
    if ( cmpxchg(&v->irq_mode, XHYPASS_IRQ_RTO,
                 XHYPASS_IRQ_TO_DYN) != XHYPASS_IRQ_RTO )
    {
        rc = -EBUSY;
        goto out_unlock;
    }
    ...
    stage = "local-active-check";
    rc = gicv3_rto_local_active();
    if ( rc )
        goto abort;
    stage = "physical-active-check";
    ...
    WRITE_SYSREG(v->arch.rto_gic.icc_igrpen1_el1, ICC_IGRPEN1_EL1);
    WRITE_SYSREG(v->arch.rto_gic.icc_pmr_el1, ICC_PMR_EL1);
    WRITE_SYSREG(v->arch.rto_gic.icc_bpr1_el1, ICC_BPR1_EL1);
    WRITE_SYSREG(v->arch.rto_gic.hcr_el2, HCR_EL2);
    WRITE_SYSREG(v->arch.rto_gic.ich_hcr_el2, ICH_HCR_EL2);
    v->arch.hcr_el2 = v->arch.rto_gic.hcr_el2;
    isb();
...
out_unlock:
    spin_unlock(&v->irq_mode_lock);
    if ( rc )
        return gicv3_rto_log_failure(v, "RTO->DYN", stage, rc);
    return rc;
}
...
static const struct gic_hw_operations gicv3_ops = {
    ...
    .enter_rto           = gicv3_enter_rto,
    .exit_rto            = gicv3_exit_rto,
};
