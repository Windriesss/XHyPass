...
int arch_vcpu_create(struct vcpu *v)
{
    ...
    spin_lock_init(&v->irq_mode_lock);
    v->irq_mode = XHYPASS_IRQ_DYN;
...
static void xhypass_update_event_irq(struct vcpu *v, bool pending)
{
    unsigned long flags;

    spin_lock_irqsave(&v->irq_mode_lock, flags);
    if ( ACCESS_ONCE(v->irq_mode) == XHYPASS_IRQ_DYN )
    ...
    else if ( pending )
    {
        ...
        send_SGI_mask(cpumask_of(v->processor),
                      (enum gic_sgi)v->rto_event_sgi);
    }
    spin_unlock_irqrestore(&v->irq_mode_lock, flags);
}
...
void vcpu_mark_events_pending(struct vcpu *v)
{
    ...
    xhypass_update_event_irq(v, true);
}
...
void vcpu_update_evtchn_irq(struct vcpu *v)
{
    ...
    xhypass_update_event_irq(v, pending);
}
