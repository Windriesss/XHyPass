...
void do_IRQ(struct cpu_user_regs *regs, unsigned int irq, int is_fiq)
{
    ...
    if ( test_bit(_IRQ_GUEST, &desc->status) )
    {
        ...
        struct vcpu *target;
        enum xhypass_irq_mode mode;

        perfc_incr(guest_irqs);
        desc->handler->end(desc);

        target = vgic_get_target_vcpu(info->d->vcpu[0], info->virq);
        mode = target ? ACCESS_ONCE(target->irq_mode) : XHYPASS_IRQ_DYN;

        /*
         * A remote pCPU may have acknowledged this interrupt immediately
         * before the target vCPU started switching away from the vGIC.  Do
         * not enqueue a late virtual interrupt after TO_RTO is published.
         * The physical priority has already been dropped by ->end(); force
         * deactivation and leave one pending instance for the new owner.
         *
         * The same rule is used during TO_DYN: target IRQs are masked by the
         * transition code, so an acknowledged straggler remains physically
         * pending until DYN ownership is committed.
         */
        if ( unlikely(mode != XHYPASS_IRQ_DYN) )
        {
        ...
        vgic_inject_irq(info->d, target, info->virq, true);
        goto out_no_end;
    }
