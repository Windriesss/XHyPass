...
static void do_trap_xhypass_hvc(struct cpu_user_regs *regs)
{
    unsigned long flags;
    long rc = -EOPNOTSUPP;

    local_irq_save(flags);
    switch ( HYPERCALL_ARG1(regs) )
    {
    case XEN_XHYPASS_ENTER_RTO:
        stop_hyp_timer();
        if ( gic_hw_ops->enter_rto )
            rc = gic_hw_ops->enter_rto(HYPERCALL_ARG2(regs));
        if ( rc )
            enable_hyp_timer();
        break;

    case XEN_XHYPASS_EXIT_RTO:
        if ( gic_hw_ops->exit_rto )
            rc = gic_hw_ops->exit_rto();
        if ( !rc )
            enable_hyp_timer();
        break;
                              ...
                              const union hsr hsr)
{
    ...
    if ( hsr.iss == XEN_HYPERCALL_XHYPASS )
    {
        do_trap_xhypass_hvc(regs);
        return;
    }
