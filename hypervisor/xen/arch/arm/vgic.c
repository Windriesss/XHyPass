...
/* Serialize SGI delivery with the DYN/RTO ownership handoff. */
static void vgic_deliver_sgi(struct vcpu *target, unsigned int virq)
{
    unsigned long flags;

    spin_lock_irqsave(&target->irq_mode_lock, flags);
    if ( ACCESS_ONCE(target->irq_mode) == XHYPASS_IRQ_RTO )
    {
        ASSERT(target->processor < nr_cpu_ids);
        send_SGI_one(target->processor, (enum gic_sgi)virq);
    }
    else
    {
        ASSERT(ACCESS_ONCE(target->irq_mode) == XHYPASS_IRQ_DYN);
        vgic_inject_irq(target->domain, target, virq, true);
    }
    spin_unlock_irqrestore(&target->irq_mode_lock, flags);
}
                 ...
                 int virq, const struct sgi_target *target)
{
    ...
    case SGI_TARGET_LIST:
        ...
        for_each_set_bit ( i, bitmap )
        {
            ...
            vgic_deliver_sgi(d->vcpu[vcpuid], virq);
        }
        break;
    case SGI_TARGET_OTHERS:
        ...
        for ( unsigned int i = 0; i < d->max_vcpus; i++ )
        {
            if ( i != current->vcpu_id && d->vcpu[i] != NULL &&
                ...
                vgic_deliver_sgi(d->vcpu[i], virq);
        }
        break;
    case SGI_TARGET_SELF:
        perfc_incr(vgic_sgi_self);
        vgic_deliver_sgi(current, virq);
        break;
    default:
        gprintk(XENLOG_WARNING,
