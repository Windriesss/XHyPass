...
cat > "${RESULT_DIR}/nonroot_config.txt" <<EOF
DURATION=${DURATION}
SEED=${SEED}
WORKERS=${WORKERS}
...
"${BENCHMARK}" --model "${MODEL}" --threads "${MODEL_THREADS}" --port 8081 \
...
run_case() {
    ...
    POISSON_QPS=$3
    CONST_QPS=$4
    echo "[NN/NONROOT] Starting ${LEVEL}"
    ...
    echo "[NN/NONROOT] Finished ${LEVEL}"
}

run_case light "${LIGHT_PEAK_QPS}" "${LIGHT_POISSON_QPS}" "${LIGHT_CONSTANT_QPS}" || exit $?
run_case medium "${MEDIUM_PEAK_QPS}" "${MEDIUM_POISSON_QPS}" "${MEDIUM_CONSTANT_QPS}" || exit $?
run_case heavy "${HEAVY_PEAK_QPS}" "${HEAVY_POISSON_QPS}" "${HEAVY_CONSTANT_QPS}" || exit $?
echo "[NN/NONROOT] All cases completed"
